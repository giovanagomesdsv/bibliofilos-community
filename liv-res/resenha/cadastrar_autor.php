...
<body>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <?php if ($result && $result->num_rows > 0): ?>
            <?php while ($row = $result->fetch_assoc()): ?>
                <div class="autor-card">
                    <input type="radio" name="autor_existente" value="<?= esc($row['aut_id']) ?>" id="autor_<?= esc($row['aut_id']) ?>">
                    <label for="autor_<?= esc($row['aut_id']) ?>">
                        <img src="../../adm/imagens/autores/<?= esc($row['aut_foto']) ?>" alt="<?= esc($row['aut_nome']) ?>">
                        <?= esc($row['aut_nome']) ?>
                    </label>
                </div>
            <?php endwhile; ?>
        ...
        <br><br>
        <input type="submit" value="Cadastrar Autor ou Relacionar Autor Existente">
    </form>
</body>
</html>
